...
public:
    ...
    py::tuple execute(std::string inputs, std::vector<std::string> outputIdentifiers)
    {
        ...
        // Cloning the programAst is necessary to be able to execute the same program multiple times (which uses the
        // same program AST)
        ...
        //
        //   if (!success)
        //     throw stork::runtime_error("Currently, only the dummy ciphertext factory and cleartext results are
        //     implemented!");
        // }
...
PYBIND11_MODULE(_abc_wrapper, m)
{
    ...
    py::class_<ABCProgramWrapper>(m, "ABCProgramWrapper")
        ...
        .def("add_fn", &ABCProgramWrapper::add_fn, "Pre-process a json string of a function and add it.")
        .def("execute", &ABCProgramWrapper::execute, "Execute the compiled program on the given inputs and outputs")
        .def("dump", &ABCProgramWrapper::dump, "Print the parsed MLIR");
}
