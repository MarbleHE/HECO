...
bool try_minimal_primitive_root(uint64_t degree,
  ...
  for (size_t i = 0; i < degree; i++) {
    ...
    ConstPointer current_generator_copy
        (duplicate_uint_if_needed(current_generator.get(), uint64_count, uint64_count, true, pool));
    multiply_uint_uint_mod(current_generator_copy.get(),
                           generator_sq.get(),
                           prime_modulus,
                           current_generator.get(),
                           pool);
  }
...
void exponentiate_uint_mod(const uint64_t *operand,
                           const uint64_t *exponent,
                           int exponent_uint64_count,
