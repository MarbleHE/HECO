...
#include "ast_opt/utilities/seal_2.3.0/util/polycore.h"
#include "ast_opt/utilities/seal_2.3.0/util/uintarith.h"
#include "ast_opt/utilities/seal_2.3.0/util/polyarith.h"
#include <algorithm>
            ...
            namespace tools
            {
                ...
                void symbolic_dft_oneroundless(uint64_t *operand, int coeff_count_power, int coeff_uint64_count, int m, int r, uint64_t *alloc_ptr)
                {
                    ...
                    for (int outer_index = outer_start; outer_index >= 0; outer_index--)
                    {
                        ...
                        {
                            int sr = reverse_bits(static_cast<uint32_t>(middle_index), outer_remaining) << outer_index; //scrambling.
                            ...
                            for (int inner_index = 0; inner_index < inner_end; inner_index++)
                            {
                                ...
                                for (int a = 0; a < r; a++)
                                {
                                    ...
                                    {
                                        borrow = sub_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr, borrow, x_coeff_ptr2++);
                                        carry = add_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr++, carry, x_coeff_ptr);
                                        x_coeff_ptr++;
                                    }
                                }
                            }
                ...
                void symbolic_inverse_dft(uint64_t *operand, int coeff_count_power, int coeff_uint64_count, int m, int r, uint64_t *alloc_ptr)
                {
                    ...
                    for (int outer_index = 0; outer_index <= outer_end; ++outer_index)
                    {
                        ...
                        {
                            int sr = reverse_bits(static_cast<uint32_t>(middle_index), outer_remaining) << outer_index;
                            ...
                            for (int inner_index = 0; inner_index < inner_end; inner_index++)
                            {
                                ...
                                for (int a = 0; a < r; a++)
                                {
                                    ...
                                    for (int add_sub_index = 1; add_sub_index < coeff_uint64_count; add_sub_index++)
                                    {
                                        borrow = sub_uint64(*coeff1_ptr, *coeff2_ptr, borrow, dest_coeff_ptr++);
                                        carry = add_uint64(*coeff1_ptr, *coeff2_ptr++, carry, coeff1_ptr);
                                        coeff1_ptr++;
                                    }
                                }
                ...
                void multiply_uint_uint_add(const uint64_t *operand1, const uint64_t *operand2, int operand_uint64_count, int result_uint64_count, uint64_t *result)
                {
                    ...
                    for (int operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        ...
                        {
                            unsigned char c_carry = add_uint64(*inner_result, carry, 0, inner_result);
                            inner_result++;
                            for (int i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result, 0, c_carry, inner_result);
                                inner_result++;
                            }
                        }
                    ...
                    uint64_t *result1, uint64_t *result2, uint64_t *result3)
                {
                    ...
                    for (int operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        const uint64_t *inner_operand12 = operand12;
                        const uint64_t *inner_operand22 = operand22;
                        const uint64_t *inner_operand32 = operand32;

                        uint64_t *inner_result1 = result1++;
                        uint64_t *inner_result2 = result2++;
                        uint64_t *inner_result3 = result3++;

                        uint64_t carry1 = 0, carry2 = 0, carry3 = 0;
                        int operand2_index = 0;
                        int operand2_index_max = min(operand_uint64_count, result_uint64_count - operand1_index);
                        for (; operand2_index < operand2_index_max; operand2_index++)
                        {
                            ...
                            multiply_uint64(*operand11, *inner_operand12++, temp_result);
                            carry1 = temp_result[1] + add_uint64(temp_result[0], carry1, 0, temp_result);
                            carry1 += add_uint64(*inner_result1, temp_result[0], 0, inner_result1);

                            multiply_uint64(*operand21, *inner_operand22++, temp_result);
                            carry2 = temp_result[1] + add_uint64(temp_result[0], carry2, 0, temp_result);
                            carry2 += add_uint64(*inner_result2, temp_result[0], 0, inner_result2);

                            multiply_uint64(*operand31, *inner_operand32++, temp_result);
                            carry3 = temp_result[0] + add_uint64(temp_result[0], carry3, 0, temp_result);
                            carry3 += add_uint64(*inner_result3, temp_result[0], 0, inner_result3);
                        ...
                        if (operand1_index + operand2_index_max < result_uint64_count)
                        {
                            unsigned char c_carry = add_uint64(*inner_result1, carry1, 0, inner_result1);
                            inner_result1++;
                            for (int i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result1, 0, c_carry, inner_result1);
                                inner_result1++;
                            }

                            c_carry = add_uint64(*inner_result2, carry2, 0, inner_result2);
                            inner_result2++;
                            for (int i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result2, 0, c_carry, inner_result2);
                                inner_result2++;
                            }

                            c_carry = add_uint64(*inner_result3, carry3, 0, inner_result3);
                            inner_result3++;
                            for (int i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result3, 0, c_carry, inner_result3);
                                inner_result3++;
                            }
                        }
                    ...
                    uint64_t *result1, uint64_t *result2, uint64_t *result3)
                {
                    ...
                    for (int operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        ...
                        for (; operand2_index < operand2_index_max; operand2_index++)
                        {
                            ...
                            multiply_uint64(*operand11, *inner_operand22, temp_result);
                            carry2 = temp_result[1] + add_uint64(temp_result[0], carry2, 0, temp_result);
                            carry2 += add_uint64(*inner_result2, temp_result[0], 0, inner_result2);

                            multiply_uint64(*operand31, *inner_operand22++, temp_result);
                            carry3 = temp_result[1] + add_uint64(temp_result[0], carry3, 0, temp_result);
                            carry3 += add_uint64(*inner_result3, temp_result[0], 0, inner_result3);
                ...
                inline void set_uint_uint_negate_if(const uint64_t *operand, int uint64_count, unsigned char negate, uint64_t *destination)
                {
                    ...
                    {
                        negate = add_uint64(*operand++ ^ negate_mask, 0, negate, destination++);
                    }
                }
            }
            ...
            namespace nussbaumer_multiply
            {
                ...
                inline void do_nussbaumer_base_signcheck(uint64_t *operand1, uint64_t *operand2, int coeff_count_power, int sum_uint64_count, int product_uint64_count, uint64_t *result, uint64_t *alloc_ptr)
                {
                    if (coeff_count_power == 2)
                    {
                        ...
                        unsigned char op_coeff_signs[2 * coeff_count]{ 0 };
                        ...
                        for (int i = 0; i < coeff_count; i++)
                        {
                            ...
                            {
                                borrow = sub_uint64(*(alloc_ptr + positive_offset), *alloc_ptr, borrow, result++);
                                alloc_ptr++;
                            }
                        }
                ...
                void do_nussbaumer_base_signcheck(uint64_t *operand1, uint64_t *operand2, int coeff_count_power, int sum_uint64_count, int product_uint64_count, uint64_t *result_11, uint64_t *result_22, uint64_t *result_12, uint64_t *alloc_ptr)
                {
                    if (coeff_count_power == 2)
                    {
                        ...
                        for (int i = 0; i < coeff_count; i++)
                        {
                            ...
                            for (int j = 0; j < product_uint64_count; j++)
                            {
                                borrow11 = sub_uint64(*(negative_result_11 + positive_offset), *negative_result_11, borrow11, result_11++);
                                negative_result_11++;
                                borrow12 = sub_uint64(*(negative_result_12 + positive_offset), *negative_result_12, borrow12, result_12++);
                                negative_result_12++;
                                borrow22 = sub_uint64(*(negative_result_22 + positive_offset), *negative_result_22, borrow22, result_22++);
                                negative_result_22++;
                            }
                        }
