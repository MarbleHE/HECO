...
class AbstractStatement : public AbstractNode {
 public:
  ...
  inline std::unique_ptr<AbstractStatement> clone(AbstractNode* parent) const {
    return std::unique_ptr<AbstractStatement>(clone_impl(parent));
  }
 private:
  /// Refines return type to AbstractStatement
  AbstractStatement *clone_impl(AbstractNode* parent) const override = 0;
};
