...
struct BinaryExpressionAcc {
  ...
  /// Indicates whether the visitor is currently in an subtree of an variable declaration (VarDecl) or variable
  /// assignment (VarAssignm). In that case, the simplified subtree to be generated by getSimplifiedSubtree() will use
  ...
  void reset() {
    ...
    isVisitingVariableDeclarationOrAssignment = false;
  }
  ...
  void addAllOperandsOfSubtree(AbstractBinaryExpr *pExpr) {
    std::vector<AbstractExpr *> operandsToAdd;
    for (auto &node : pExpr->getDescendants()) {
      if (node->countChildrenNonNull()==0
          && (dynamic_cast<AbstractLiteral *>(node)!=nullptr || dynamic_cast<Variable *>(node)!=nullptr)) {
        operandsToAdd.push_back(node->castTo<AbstractExpr>());
      }
    }
    addOperands(operandsToAdd);
  }
};
