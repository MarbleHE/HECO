...
class OpSymb {
 public:
  ...
  static AbstractLiteral *getIdentityElement(std::variant<OpSymb::ArithmeticOp,
                                                          OpSymb::LogCompOp,
                                                          OpSymb::UnaryOp> op) {
    if (op.index()==0) {  // op is ArithmeticOp
      auto arithmeticOp = std::get<OpSymb::ArithmeticOp>(op);
      ...
      auto logCompOp = std::get<OpSymb::LogCompOp>(op);
