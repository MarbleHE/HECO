...
/// forward declaration of Visitor interface from heco/visitor/IVisitor.h
class IVisitor;
...
private:
    ...
    /// and introduce a std::unique_ptr<DerivedNode> clone() method that hides AbstractNode::clone() (for use with
    /// derived class ptrs/refs) WARNING: This method should never be called outside of this clone() method for memory
    /// safety reasons \return A clone of the node including clones of all of its children.
...
protected:
    ...
    /// Returns the node's type, which is the name of the object in the AST. This method must be overridden by all
    /// classes that inherit from AbstractNode by their respective name (e.g., ArithmeticExp, Function, Variable).
...
private:
    ...
    /// A static ongoing counter that is incremented after creating a new AbstractNode object. The counter's value is
    /// used to build the unique node ID.
