...
public:
    /// A typedef that allows obtaining the type T from any Literal<T> class, e.g., LiteralBool::value_type returns
    /// bool.
...
#include "heco/legacy_ast/ast_utilities/warning_hidingNonVirtualFunction_prologue.h"
...
{
    return (dynamic_cast<LiteralBool *>(&node) != nullptr) || (dynamic_cast<LiteralChar *>(&node) != nullptr) ||
           (dynamic_cast<LiteralInt *>(&node) != nullptr) || (dynamic_cast<LiteralFloat *>(&node) != nullptr) ||
           (dynamic_cast<LiteralDouble *>(&node) != nullptr) || (dynamic_cast<LiteralString *>(&node) != nullptr);
}

// Pretty "getNodeType()" specializations for the common types:

template <>
inline std::string Literal<bool>::getNodeType() const
{
    return "LiteralBool";
};

template <>
inline std::string Literal<char>::getNodeType() const
{
    return "LiteralChar";
};

template <>
inline std::string Literal<int>::getNodeType() const
{
    return "LiteralInt";
};

template <>
inline std::string Literal<float>::getNodeType() const
{
    return "LiteralFloat";
};

template <>
inline std::string Literal<double>::getNodeType() const
{
    return "LiteralDouble";
};

template <>
inline std::string Literal<std::string>::getNodeType() const
{
