...
void ConeRewriter::rewriteCones(Ast &astToRewrite, const std::vector<Node *> &coneEndNodes) {
  ...
  for (auto coneEnd : coneEndNodes) {
    ...
    // TODO(pjattke): consider case in which there are no XOR nodes in between coneStart and coneEnd
...
//    while (currentNode != xorEndNode) {
    while (true) {
      ...
      currentNode = critIn;
      // termination criteria: we reached the end of the XOR chain
      if (currentNode == xorEndNode) {
        ...
        if (isCriticalNode(xorEndNodeAsLogicalExpr->getLeft()) != isCriticalNode(xorEndNodeAsLogicalExpr->getRight())) {
