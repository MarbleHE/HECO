...
namespace stork
{
    Error::Error(std::string message, size_t line_number, size_t char_index) noexcept
        : _message(std::move(message)), _line_number(line_number), _char_index(char_index)
    {
        _long_message =
            _message + " on Line " + std::to_string(_line_number) + ", position " + std::to_string(_char_index);
    }
    ...
    Error wrongTypeError(
        std::string_view source, std::string_view destination, bool lvalue, size_t line_number, size_t char_index)
    {
        std::string message;
        if (lvalue)
        {
