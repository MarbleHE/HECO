...
bool isBinaryOperator(const stork::tokens_iterator &it)
{
    return it->isReservedToken() &&
           (it->hasValue(stork::reservedTokens::add) || it->hasValue(stork::reservedTokens::sub) ||
            it->hasValue(stork::reservedTokens::concat) || it->hasValue(stork::reservedTokens::mul) ||
            it->hasValue(stork::reservedTokens::div) || it->hasValue(stork::reservedTokens::idiv) ||
            it->hasValue(stork::reservedTokens::mod) || it->hasValue(stork::reservedTokens::bitwise_and) ||
            it->hasValue(stork::reservedTokens::bitwise_or) || it->hasValue(stork::reservedTokens::bitwise_xor) ||
            it->hasValue(stork::reservedTokens::shiftl) || it->hasValue(stork::reservedTokens::shiftr) ||
            it->hasValue(stork::reservedTokens::logical_and) || it->hasValue(stork::reservedTokens::logical_or) ||
            it->hasValue(stork::reservedTokens::eq) || it->hasValue(stork::reservedTokens::ne) ||
            it->hasValue(stork::reservedTokens::lt) || it->hasValue(stork::reservedTokens::gt) ||
            it->hasValue(stork::reservedTokens::le) || it->hasValue(stork::reservedTokens::ge) ||
            it->hasValue(stork::reservedTokens::fhe_add) || it->hasValue(stork::reservedTokens::fhe_sub) ||
...
{
    return it->isReservedToken() &&
           (it->hasValue(stork::reservedTokens::logical_not) || it->hasValue(stork::reservedTokens::bitwise_not));
}

bool isPostFixOperator(const stork::tokens_iterator &it)
{
    return it->isReservedToken() &&
           (it->hasValue(stork::reservedTokens::inc) || it->hasValue(stork::reservedTokens::dec));
}
...
AbstractExpression *Parser::parseExpression(stork::tokens_iterator &it)
{
    ...
    while (running)
    {
        ...
        else if (isPostFixOperator(it))
        {
            ...
            if (operands.empty())
            {
                throw stork::expectedSyntaxError(
    ...
    while (!operator_stack.empty())
    {
        ...
        else
        {
            ...
            if (operands.size() < 2)
            {
                throw stork::unexpectedSyntaxError(
