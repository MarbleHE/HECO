...
#include "heco/ast_utilities/NodeUtils.h"
...
{
  return it->isReservedToken() && (it->hasValue(stork::reservedTokens::inc) || it->hasValue(stork::reservedTokens::dec));
}

bool isLiteral(stork::tokens_iterator &it)
{
  return it->isBool() || it->isChar() || it->isFloat() || it->isDouble() || it->isInteger() || it->isString() || it->hasValue(stork::reservedTokens::kw_true) || it->hasValue(stork::reservedTokens::kw_false);
}
...
AbstractExpression *Parser::parseExpression(stork::tokens_iterator &it)
{
  ...
  while (running)
  {
    ...
    else if (isLiteral(it))
    {
      ...
      if (operands.empty() && !operator_stack.empty() && operator_stack.top().toString() == Operator(SUBTRACTION).toString())
      {
