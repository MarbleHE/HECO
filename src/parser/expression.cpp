              ...
              std::is_same<To, string>::value &&
                  (
                      std::is_same<From, number>::value ||
                          ...
                          std::is_same<From, integer>::value ||
                          std::is_same<From, linteger>::value
                  )
          ) ||
            ...
            [&](simple_type st) {\
                switch (st) {      \
                    case simple_type::integer:\
                        return expression_ptr(std::make_unique<tostring_expression<R, int> > (\
                            expression_builder<int>::build_expression(np->get_children()[0], context)\
                        ));\
...
class expression_builder {
 private:
  ...
  static expression_ptr build_integer_expression(const node_ptr &np, compiler_context &context) {
    if (std::holds_alternative<int>(np->get_value())) {
      return std::make_unique<constant_expression<R, integer>>(
          std::get<int>(np->get_value())
      );
    }

    CHECK_IDENTIFIER(linteger);

    switch (std::get<node_operation>(np->get_value())) {
      CHECK_UNARY_OPERATION(postinc, linteger);
      CHECK_UNARY_OPERATION(postdec, linteger);
      CHECK_UNARY_OPERATION(positive, integer);
      CHECK_UNARY_OPERATION(negative, integer);
      CHECK_UNARY_OPERATION(bnot, integer);
      CHECK_UNARY_OPERATION(lnot, integer);
      CHECK_SIZE_OPERATION();
      CHECK_BINARY_OPERATION(add, integer, integer);
      CHECK_BINARY_OPERATION(sub, integer, integer);
      CHECK_BINARY_OPERATION(mul, integer, integer);
      CHECK_BINARY_OPERATION(div, integer, integer);
      CHECK_BINARY_OPERATION(idiv, integer, integer);
      CHECK_BINARY_OPERATION(mod, integer, integer);
      CHECK_BINARY_OPERATION(band, integer, integer);
      CHECK_BINARY_OPERATION(bor, integer, integer);
      CHECK_BINARY_OPERATION(bxor, integer, integer);
      CHECK_BINARY_OPERATION(bsl, integer, integer);
      CHECK_BINARY_OPERATION(bsr, integer, integer);
      CHECK_COMPARISON_OPERATION(eq);
      CHECK_COMPARISON_OPERATION(ne);
      CHECK_COMPARISON_OPERATION(lt);
      CHECK_COMPARISON_OPERATION(gt);
      CHECK_COMPARISON_OPERATION(le);
      CHECK_COMPARISON_OPERATION(ge);
      CHECK_BINARY_OPERATION(comma, void, integer);
      CHECK_BINARY_OPERATION(land, integer, integer);
      CHECK_BINARY_OPERATION(lor, integer, integer);
      CHECK_INDEX_OPERATION(integer, array);
      CHECK_TERNARY_OPERATION(ternary, integer, integer, integer);
      CHECK_CALL_OPERATION(integer);
      default: throw expression_builder_error();
    }
  }
  ...
  static expression_ptr build_linteger_expression(const node_ptr &np, compiler_context &context) {
    CHECK_IDENTIFIER(linteger);

    switch (std::get<node_operation>(np->get_value())) {
      CHECK_UNARY_OPERATION(preinc, linteger);
      CHECK_UNARY_OPERATION(predec, linteger);
      CHECK_BINARY_OPERATION(assign, linteger, integer);
      CHECK_BINARY_OPERATION(add_assign, linteger, integer);
      CHECK_BINARY_OPERATION(sub_assign, linteger, integer);
      CHECK_BINARY_OPERATION(mul_assign, linteger, integer);
      CHECK_BINARY_OPERATION(div_assign, linteger, integer);
      CHECK_BINARY_OPERATION(idiv_assign, linteger, integer);
      CHECK_BINARY_OPERATION(mod_assign, linteger, integer);
      CHECK_BINARY_OPERATION(band_assign, linteger, integer);
      CHECK_BINARY_OPERATION(bor_assign, linteger, integer);
      CHECK_BINARY_OPERATION(bxor_assign, linteger, integer);
      CHECK_BINARY_OPERATION(bsl_assign, linteger, integer);
      CHECK_BINARY_OPERATION(bsr_assign, linteger, integer);
      CHECK_BINARY_OPERATION(comma, void, linteger);
      CHECK_INDEX_OPERATION(linteger, larray);
      CHECK_TERNARY_OPERATION(ternary, integer, linteger, linteger);
      default: throw expression_builder_error();
    }
  }
    ...
    return std::visit(overloaded{
        [&](simple_type st) {
          switch (st) {
            case simple_type::integer:
              if (np->is_lvalue()) {
                RETURN_EXPRESSION_OF_TYPE(linteger);
              } else {
                RETURN_EXPRESSION_OF_TYPE(integer);
              }
            case simple_type::number:
              if (np->is_lvalue()) {
  ...
  return std::visit(overloaded{
      [&](simple_type st) {
        switch (st) {
          ...
          case simple_type::integer: return expression_builder<int>::build_param_expression(np, context);
          case simple_type::string: return expression_builder<string>::build_param_expression(np, context);
          case simple_type::nothing: throw expression_builder_error();
...
template<typename R>
typename expression<R>::ptr build_expression(type_handle type_id,
...
expression<integer>::ptr build_integer_expression(compiler_context &context, tokens_iterator &it) {
  return build_expression<integer>(type_registry::get_integer_handle(), context, it, true);
}
...
expression<lvalue>::ptr build_default_initialization(type_handle type_id) {
  return std::visit(overloaded{
      [&](simple_type st) {
        switch (st) {
          case simple_type::integer:
            return expression<lvalue>::ptr(std::make_unique<default_initialization_expression < integer> > ());
          case simple_type::number:
