  ...
  return std::visit(overloaded{
      [](reserved_token t) {
        switch (t) {
          ...
          case reserved_token::kw_int:
          case reserved_token::kw_string:
          case reserved_token::kw_void:
          case reserved_token::open_square: return true;
...
statement_ptr compile_statement(compiler_context &ctx, tokens_iterator &it, possible_flow pf, bool in_switch) {
  if (it->is_reserved_token()) {
    switch (it->get_reserved_token()) {
      case reserved_token::kw_for: return compile_for_statement(ctx, it, pf.add_loop());
      case reserved_token::kw_while: return compile_while_statement(ctx, it, pf.add_loop());
      case reserved_token::kw_do: return compile_do_statement(ctx, it, pf.add_loop());
      case reserved_token::kw_if: return compile_if_statement(ctx, it, pf);
      case reserved_token::kw_switch: return compile_switch_statement(ctx, it, pf.add_switch());
      case reserved_token::kw_break: return compile_break_statement(ctx, it, pf);
      case reserved_token::kw_continue: return compile_continue_statement(ctx, it, pf);
      case reserved_token::kw_return:return compile_return_statement(ctx, it, pf);
      default: break;
    }
  }
...
statement_ptr compile_for_statement(compiler_context &ctx, tokens_iterator &it, possible_flow pf) {
  ...
  // for-statement's block (e.g., for (...) { stmt1; stmt2; ... })
...
type_handle parse_type(compiler_context &ctx, tokens_iterator &it) {
  ...
  switch (it->get_reserved_token()) {
    case reserved_token::kw_void: t = ctx.get_handle(simple_type::nothing);
      ++it;
      break;
    case reserved_token::kw_number: t = ctx.get_handle(simple_type::number);
      ++it;
      break;
    case reserved_token::kw_int: t = ctx.get_handle(simple_type::integer);
      ++it;
      break;
    case reserved_token::kw_string: t = ctx.get_handle(simple_type::string);
      ++it;
      break;
    case reserved_token::open_square: {
      ...
      break;
    default: throw unexpected_syntax(it);
  }

  while (it->is_reserved_token()) {
    switch (it->get_reserved_token()) {
      case reserved_token::open_square: parse_token_value(ctx, ++it, reserved_token::close_square);
...
runtime_context compile(
  ...
  while (it) {
    ...
    switch (it->get_reserved_token()) {
      case reserved_token::kw_public: public_function = true;
  ...
  // TODO return AST in context object
