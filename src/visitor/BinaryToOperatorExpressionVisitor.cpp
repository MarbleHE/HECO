...
void SpecialBinaryToOperatorExpressionVisitor::visit(BinaryExpression &elem) {
  ...
  // Now, the visit call from which this was called (i.e. visit parent of elem) needs to replace elem with this new expression
  // TODO: Doesn't work if elem is the root, since then there is no visit call, just a random accept
  //       This can be solved be encapsulating this visitor inside a wrapper class that replaces the root!
}
