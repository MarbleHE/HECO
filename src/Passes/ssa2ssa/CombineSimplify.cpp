...
LogicalResult simplify(IRRewriter &rewriter, MLIRContext *context, fhe::CombineOp& op) {
  ...
  collectInputs(op, single_inputs, remaining_inputs);
  ...
  if (updated) {
    ...
    op.vectorsMutable().assign(new_vectors);
    op.getOperation()->setAttr("indices", ArrayAttr::get(context, new_indices));
    //op->print(llvm::outs());
    //llvm::outs() << "\n";
    //llvm::outs().flush();
  } else {
...
void CombineSimplifyPass::runOnOperation() {
  ...
  for (auto f: llvm::make_early_inc_range(block.getOps<FuncOp>())) {
    ...
    if (f.walk([&](Operation *op) {
      if (auto combine_op = llvm::dyn_cast_or_null<fhe::CombineOp>(op))
        if (simplify(rewriter, &getContext(), combine_op).failed())
