...
mlir::Block &getBlock(Region &region_containing_blockop)
{
    ...
    {
        emitError(
            region_containing_blockop.getLoc(),
    ...
    {
        emitError(
            region_containing_blockop.getLoc(), "Expected this region to contain an abc.block but it contained an " +
...
mlir::Value translateExpression(Operation &op, IRRewriter &rewriter, CustomSymbolTable &symbolTable)
{
    if (auto literal_int = llvm::dyn_cast<ast::LiteralIntOp>(op))
    {
        // Literal Ints are created as "Index" to start with (since it's most common) and should later be converted if
        // needed
        auto value = rewriter.create<arith::ConstantOp>(
            op.getLoc(), rewriter.getIndexAttr(literal_int.value().getLimitedValue()));
        return value;
    }
    else if (auto literal_tensor = llvm::dyn_cast<ast::LiteralTensorOp>(op))
    {
        ...
        auto value = rewriter.create<arith::ConstantOp>(op.getLoc(), rewriter.getIndexTensorAttr(stuff));
        return value;
    }
    else if (auto variable = llvm::dyn_cast<ast::VariableOp>(op))
    {
        if (!symbolTable.count(variable.name()))
        {
            ...
            return rewriter.create<arith::ConstantOp>(
                op.getLoc(), rewriter.getIntegerAttr(rewriter.getIntegerType(64), 1));
        }
        else
        {
    ...
    else if (auto index_access = llvm::dyn_cast<ast::IndexAccessOp>(op))
    {
        ...
        else if (auto target_ia = llvm::dyn_cast<ast::IndexAccessOp>(firstOp(index_access.target())))
        {
            ...
            {
                emitError(
                    op.getLoc(), "Expected Index Access target to be nested once or a  variable, got " +
...
void translateStatement(
    Operation &op, IRRewriter &rewriter, CustomSymbolTable &symbolTable, AffineForOp *current_for_op = nullptr,
...
void translateIfOp(ast::IfOp &if_op, IRRewriter &rewriter, CustomSymbolTable &symbolTable)
{
    ...
    std::function<void(const std::string &, Value, Value)> then_callback = [&](const std::string &name, Value oldValue,
                                                                               Value newValue) {
    ...
    std::function<void(const std::string &, Value, Value)> else_callback = [&](const std::string &name, Value oldValue,
                                                                               Value newValue) {
    ...
    for (const auto &p : updated_values)
    {
        ...
        if (updated_in_then)
        {
            condThenValue = rewriter.create<fhe::MultiplyOp>(
                if_op->getLoc(), ValueRange({ condition, then_values.find(name)->second }));
        }
        if (updated_in_else)
        {
            condElseValue = rewriter.create<fhe::MultiplyOp>(
                if_op->getLoc(), ValueRange({ condition, else_values.find(name)->second }));
        }
...
void translateVariableDeclarationOp(
    ast::VariableDeclarationOp vardecl_op, IRRewriter &rewriter, CustomSymbolTable &symbolTable)
{
    ...
    if (value.getType() != type)
    {
        if (auto secret_type = type.cast<fhe::SecretType>())
        {
            ...
            {
                // If this was initialized with a literal, we can coerce the type: //TODO: add checks before type
                // coercion
                value = rewriter.create<fhe::ConstOp>(
                    vardecl_op->getLoc(), type_to_attr(secret_type.getPlaintextType(), const_op.getValue()));
            }
        }
        else
        {
...
void translateAssignmentOp(
    ast::AssignmentOp assignment_op, IRRewriter &rewriter, CustomSymbolTable &symbolTable, AffineForOp *for_op,
    std::function<void(const std::string &, Value, Value)> *if_op_callback)
{
    ...
    else if (auto index_access = llvm::dyn_cast<ast::IndexAccessOp>(targetOp))
    {
        if (auto target_variable = llvm::dyn_cast<ast::VariableOp>(firstOp(index_access.target())))
        {
            // if this is an index access, we need to first insert an operation, then update table with that result
            // value instead, we need to insert an operation and then update the value
            target_name = target_variable.name();
            auto index = translateExpression(firstOp(index_access.index()), rewriter, symbolTable);
            value = rewriter.create<tensor::InsertOp>(
                assignment_op->getLoc(), value, symbolTable.lookup(target_name).second, index);
            if (if_op_callback)
            {
                if_op_callback->operator()(target_name.str(), symbolTable.lookup(target_name).second, value);
            }
        ...
        {
            emitError(
                assignment_op.getLoc(), "Expected Index Access target to be a variable, got " +
                                            firstOp(index_access.target()).getName().getStringRef());
        }
    }
    else
    ...
    if (for_op)
    {
        ...
        //  SO FOR NOW WE JUST PUT EVERYTHING IN WHEN WE GENERATE A FOR OP AND LET THE CANONICALIZATION GET RID OF
        //  UNNEEDED ONES check if the symbol table still contains the symbol at the parent scope. If yes, then it's not
        //  loop local and we need to do some yield stuff! Next, we should check if it's already been added to the
        //  iter_args! by checking if one of the iter args is the same value as the one we get by looking up the old
        //  value Finally, if we ARE updating an existing iter arg, we need to find the existing yield stmt and change
        //  it otherwise, we can just emit a new yield at the end of the loop However, this might be BAD in terms of
        //  iterator stuff since we're currently in an llvm:: make early inc range thing iterating over all the ops
        //  nested in this for op!
        // emitError(assignment_op->getLoc(), "Currently, we do not handle writing to variables in for loops
        // correctly");
...
void translateSimpleForOp(ast::SimpleForOp &simple_for_op, IRRewriter &rewriter, CustomSymbolTable &symbolTable)
{
    ...
    // Create a new scope
    {
        ...
        auto new_for = rewriter.create<AffineForOp>(
            simple_for_op->getLoc(), simple_for_op.start().getLimitedValue(), simple_for_op.end().getLimitedValue(),
            1, // step size
            iter_arg_values);
        new_for_ptr = &new_for;
        ...
        else
        {
            ...
            {
                emitError(
                    abc_block.getLoc(), "ABC BlockOp must contain exactly one region and exactly one Block in that!");
            }
            else
            {
...
void translateStatement(
    Operation &op, IRRewriter &rewriter, CustomSymbolTable &symbolTable, AffineForOp *current_for_op,
    std::function<void(const std::string &, Value, Value)> *if_op_callback)
{
    rewriter.setInsertionPoint(&op);
    if (auto block_op = llvm::dyn_cast<ast::BlockOp>(op))
    {
        // TODO: Support nested ast::BlockOp in ast2hir
    ...
    else if (auto return_op = llvm::dyn_cast<ast::ReturnOp>(op))
    {
        ...
        else if (return_op.getNumRegions() == 1)
        {
            ...
            rewriter.create<mlir::func::ReturnOp>(
                op.getLoc(), translateExpression(return_value_expr, rewriter, symbolTable));
        }
        else
        {
            // TODO: Support multiple return values from ast::ReturnOp in ast2hir
    ...
    {
        // TODO: Support general ast::ForOp in ast2hir
    ...
    {
        emitError(
            op.getLoc(), "Unexpected Op encountered while translating ABC statements: " + op.getName().getStringRef());
    }
}
...
/// \param rewriter
/// \param symbolTable
void convertFunctionOp2FuncOp(ast::FunctionOp &f, IRRewriter &rewriter, CustomSymbolTable &symbolTable)
{
    ...
    for (auto pair : llvm::zip(f.getRegion(0).getOps<ast::FunctionParameterOp>(), entryBlock->getArguments()))
    {
        ...
        {
            mlir::emitError(
                arg.getLoc(), "Cannot translate FunctionParameter " + param_name + ": name is already taken.");
        }
    }
...
void heco::LowerASTtoHIRPass::runOnOperation()
{
...
void heco::LowerASTtoHIRPass::getDependentDialects(DialectRegistry &registry) const
{
    registry.insert<
