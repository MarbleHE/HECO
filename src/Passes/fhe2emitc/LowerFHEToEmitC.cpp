...
{
    registry.insert<
        emitc::EmitCDialect, fhe::FHEDialect, affine::AffineDialect, scf::SCFDialect, tensor::TensorDialect,
        func::FuncDialect>();
}
...
void LowerFHEToEmitCPass::runOnOperation()
{
        ...
        if (auto ot = t.dyn_cast_or_null<emitc::OpaqueType>())
        {
            ...
            if (old_type.dyn_cast_or_null<fhe::BatchedSecretType>() || old_type.dyn_cast_or_null<fhe::SecretType>())
                if (ot.getValue().str() == "seal::Ciphertext")
                    return std::optional<Value>(builder.create<fhe::MaterializeOp>(loc, ot, vs));
        }
    ...
    patterns.add<
        EmitCReturnPattern, EmitCArithmeticPattern<fhe::SubOp>, EmitCArithmeticPattern<fhe::AddOp>,
        EmitCArithmeticPattern<fhe::MultiplyOp>, EmitCRotatePattern, EmitCCombinePattern, EmitCFunctionPattern>(
