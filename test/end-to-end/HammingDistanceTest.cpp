...
{
    if (x.size() != y.size())
        throw std::runtime_error("Vectors  in hamming distance must have the same length.");
    int sum = 0;
...
#ifdef HAVE_SEAL_BFV
TEST(HammingDistanceTest, NaiveClear_Porcupine_Equivalence)
{ /* NOLINT */
...
TEST(HammingDistanceTest, NaiveClear_Batched_Equivalence)
{ /* NOLINT */
    // Create two vectors of bits (booleans),
    // TODO: Create test values from fixed random seed
    std::vector<bool> a(4, 0);
...
TEST(HammingDistanceTest, NaiveClear_StupidBatched_Equivalence)
{ /* NOLINT */
    // Create two vectors of bits (booleans),
    // TODO: Create test values from fixed random seed
    std::vector<bool> a(4, 0);
