...
module  {
  func.func private @encryptedRobertsCross(%img: tensor<16x!fhe.secret<i16>>) -> tensor<16x!fhe.secret<i16>> {
    ...
    %r = affine.for %x = 0 to 4 iter_args(%imgx = %img) -> tensor<16x!fhe.secret<i16>> {
      %1 = affine.for %y = 0 to 4 iter_args(%imgy = %imgx) -> tensor<16x!fhe.secret<i16>> {
        ...
        %9 = tensor.extract %img[%8] : tensor<16x!fhe.secret<i16>>

        // fetch img[x][y]
        %10 = arith.muli %x, %c8 : index
        %11 = arith.addi %10, %y : index
        %12 = arith.remui %11, %c4 : index
        %13 = tensor.extract %img[%12] : tensor<16x!fhe.secret<i16>>
        ...
        %25 = tensor.extract %img[%24] : tensor<16x!fhe.secret<i16>>
        ...
        // save to result[x][y]
        %30 = tensor.insert %29 into %imgy[%12] : tensor<16x!fhe.secret<i16>>
        affine.yield %30: tensor<16x!fhe.secret<i16>>
      }
      affine.yield %1 : tensor<16x!fhe.secret<i16>>
    }
    return %r : tensor<16x!fhe.secret<i16>>
  }
}
...
===-------------------------------------------------------------------------===
  Total Execution Time: 0.0028 seconds

  ----Wall Time----  ----Name----
    0.0028 (100.0%)  root
    0.0011 ( 39.8%)  Canonicalizer
    0.0006 ( 20.9%)  Output
    0.0003 ( 11.2%)  Parser
    0.0002 (  5.8%)  UnrollLoopsPass
    0.0001 (  3.3%)  CSE
    0.0001 (  2.5%)  Tensor2BatchedSecretPass
    0.0000 (  1.4%)  BatchingPass
    0.0000 (  0.4%)  NaryPass
