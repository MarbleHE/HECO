...
module  {
  func.func private @encryptedRobertsCross(%img: tensor<64x!fhe.secret<i16>>) -> tensor<64x!fhe.secret<i16>> {
    ...
    %r = affine.for %x = 0 to 8 iter_args(%imgx = %img) -> tensor<64x!fhe.secret<i16>> {
      %1 = affine.for %y = 0 to 8 iter_args(%imgy = %imgx) -> tensor<64x!fhe.secret<i16>> {
        ...
        %9 = tensor.extract %img[%8] : tensor<64x!fhe.secret<i16>>

        // fetch img[x][y]
        %10 = arith.muli %x, %c8 : index
        %11 = arith.addi %10, %y : index
        %12 = arith.remui %11, %c64 : index
        %13 = tensor.extract %img[%12] : tensor<64x!fhe.secret<i16>>
        ...
        %25 = tensor.extract %img[%24] : tensor<64x!fhe.secret<i16>>
        ...
        // save to result[x][y]
        %30 = tensor.insert %29 into %imgy[%12] : tensor<64x!fhe.secret<i16>>
        affine.yield %30: tensor<64x!fhe.secret<i16>>
      }
      affine.yield %1 : tensor<64x!fhe.secret<i16>>
    }
    return %r : tensor<64x!fhe.secret<i16>>
  }
}
...
===-------------------------------------------------------------------------===
  Total Execution Time: 0.0094 seconds

  ----Wall Time----  ----Name----
    0.0094 (100.0%)  root
    0.0064 ( 67.7%)  Canonicalizer
    0.0006 (  6.4%)  CSE
    0.0005 (  5.8%)  UnrollLoopsPass
    0.0005 (  5.1%)  BatchingPass
    0.0003 (  3.3%)  Parser
    0.0003 (  3.0%)  Output
    0.0003 (  2.7%)  Tensor2BatchedSecretPass
    0.0001 (  0.8%)  NaryPass
    0.0001 (  0.7%)  CombineSimplifyPass
