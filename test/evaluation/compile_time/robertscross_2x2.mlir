...
module  {
  func.func private @encryptedRobertsCross(%img: tensor<4x!fhe.secret<i16>>) -> tensor<4x!fhe.secret<i16>> {
    ...
    %r = affine.for %x = 0 to 2 iter_args(%imgx = %img) -> tensor<4x!fhe.secret<i16>> {
      %1 = affine.for %y = 0 to 2 iter_args(%imgy = %imgx) -> tensor<4x!fhe.secret<i16>> {
        ...
        %9 = tensor.extract %img[%8] : tensor<4x!fhe.secret<i16>>

        // fetch img[x][y]
        %10 = arith.muli %x, %c2 : index
        %11 = arith.addi %10, %y : index
        %12 = arith.remui %11, %c4 : index
        %13 = tensor.extract %img[%12] : tensor<4x!fhe.secret<i16>>
        ...
        // save to result[x][y]
        %30 = tensor.insert %29 into %imgy[%12] : tensor<4x!fhe.secret<i16>>
        affine.yield %30: tensor<4x!fhe.secret<i16>>
      }
      affine.yield %1 : tensor<4x!fhe.secret<i16>>
    }
    return %r : tensor<4x!fhe.secret<i16>>
  }
}
...
===-------------------------------------------------------------------------===
  Total Execution Time: 0.0018 seconds

  ----Wall Time----  ----Name----
    0.0018 (100.0%)  root
    0.0006 ( 33.1%)  Canonicalizer
    0.0003 ( 17.6%)  Output
    0.0003 ( 16.8%)  Parser
    0.0001 (  3.5%)  UnrollLoopsPass
    0.0001 (  3.1%)  CSE
    0.0000 (  2.7%)  Tensor2BatchedSecretPass
    0.0000 (  2.1%)  BatchingPass
    0.0000 (  0.5%)  NaryPass
    0.0000 (  0.3%)  CombineSimplifyPass
    0.0000 (  0.1%)  InternalOperandBatchingPass
