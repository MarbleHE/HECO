...
module  {
  func.func private @encryptedRobertsCross(%img: tensor<1024x!fhe.secret<i16>>) -> tensor<1024x!fhe.secret<i16>> {
    ...
    %r = affine.for %x = 0 to 32 iter_args(%imgx = %img) -> tensor<1024x!fhe.secret<i16>> {
      %1 = affine.for %y = 0 to 32 iter_args(%imgy = %imgx) -> tensor<1024x!fhe.secret<i16>> {
        ...
        %8 = arith.remui %7, %c1024 : index
        %9 = tensor.extract %img[%8] : tensor<1024x!fhe.secret<i16>>

        // fetch img[x][y]
        %10 = arith.muli %x, %c32 : index
        %11 = arith.addi %10, %y : index
        %12 = arith.remui %11, %c1024 : index
        %13 = tensor.extract %img[%12] : tensor<1024x!fhe.secret<i16>>
        ...
        // fetch img[x][y-1]
        %21 = arith.muli %x, %c32 : index
        %22 = arith.addi %y, %c-1 : index
        %23 = arith.addi %21, %22 : index
        %24 = arith.remui %23, %c1024 : index
        %25 = tensor.extract %img[%24] : tensor<1024x!fhe.secret<i16>>
        ...
        // save to result[x][y]
        %30 = tensor.insert %29 into %imgy[%12] : tensor<1024x!fhe.secret<i16>>
        affine.yield %30: tensor<1024x!fhe.secret<i16>>
      }
      affine.yield %1 : tensor<1024x!fhe.secret<i16>>
    }
    return %r : tensor<1024x!fhe.secret<i16>>
  }
}
...
===-------------------------------------------------------------------------===
  Total Execution Time: 0.1543 seconds

  ----Wall Time----  ----Name----
    0.1543 (100.0%)  root
    0.1207 ( 78.2%)  Canonicalizer
    0.0087 (  5.6%)  UnrollLoopsPass
    0.0086 (  5.6%)  CSE
    0.0079 (  5.1%)  BatchingPass
    0.0038 (  2.5%)  Tensor2BatchedSecretPass
    0.0022 (  1.4%)  CombineSimplifyPass
    0.0014 (  0.9%)  NaryPass
    0.0003 (  0.2%)  Parser
    0.0003 (  0.2%)  Output
