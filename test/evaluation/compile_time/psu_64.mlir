...
module  {
  func.func private @encryptedPSU(%a_id: tensor<64x8x!fhe.secret<i16>>, %a_data: tensor<64x!fhe.secret<i16>>,
                                  %b_id: tensor<64x8x!fhe.secret<i16>>, %b_data: tensor<64x!fhe.secret<i16>>) -> !fhe.secret<i16> {
    %c0_si16 = fhe.constant 0 : i16
    %c1_si16 = fhe.constant 1 : i16

    // First, sum all of A
    %1 = affine.for %i = 0 to 64 iter_args(%cur0 = %c0_si16) -> (!fhe.secret<i16>) {
        %1 = tensor.extract %a_data[%i] : tensor<64x!fhe.secret<i16>>
    ...
    %3 = affine.for %i = 0 to 64 iter_args(%cur1 = %1) -> (!fhe.secret<i16>) {
        // check if a[i] is a dupe of b[j]
        %4 = affine.for %j = 0 to 64 iter_args(%cur2 = %c1_si16) -> !fhe.secret<i16>{
            // compute a_id[i] == b_id[j]
            %5 =  affine.for %k = 0 to 8 iter_args(%cur3 = %c1_si16) -> !fhe.secret<i16>{
                %6 = tensor.extract %a_id[%i,%k] : tensor<64x8x!fhe.secret<i16>>
                %7 = tensor.extract %b_id[%j,%k] : tensor<64x8x!fhe.secret<i16>>
        ...
        %14 = tensor.extract %b_data[%i] : tensor<64x!fhe.secret<i16>>
