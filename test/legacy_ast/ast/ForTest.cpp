...
{ /* NOLINT */
    // TODO: When copying a For, the new object should contain a (deep) copy of the initializer, condition, update and
    // body
}
...
{ /* NOLINT */
    ...
    auto initializer = std::make_unique<VariableDeclaration>(
    ...
    auto condition = std::make_unique<BinaryExpression>(
        std::make_unique<Variable>("i"), Operator(LESS), std::make_unique<LiteralInt>(100));

    auto updater = std::make_unique<Assignment>(
    ...
    auto bodyStmt = std::make_unique<VariableDeclaration>(
        Datatype(Type::INT, false), std::make_unique<Variable>("dummy"), std::make_unique<LiteralInt>(111));
    auto bodyBlock = std::make_unique<Block>(std::move(bodyStmt));

    auto forLoop = std::make_unique<For>(
        std::move(initializerBlock), std::move(condition), std::move(updaterBlock), std::move(bodyBlock));
