...
TEST(VariableDeclarationTest, values_CopyAssignmentCopiesValue)
{
    ...
    // Copy the declaration
    VariableDeclaration declaration_copy(
        Datatype(Type::INT), std::make_unique<Variable>("boo"), std::make_unique<LiteralBool>(false));
...
TEST(VariableDeclarationTest, values_MoveAssignmentPreservesValue)
{
    ...
    // Move the declaration
    VariableDeclaration new_declaration(
        Datatype(Type::INT), std::make_unique<Variable>("boo"), std::make_unique<LiteralBool>(false));
...
{ /* NOLINT */
    ...
    auto *var = new VariableDeclaration(
        datatype, std::make_unique<Variable>(identifier), std::make_unique<LiteralInt>(initializer));
    nlohmann::json j = { { "type", "VariableDeclaration" },
