...
{
    {
        if (typeid(ast1) != typeid(ast2))
        {
            return ::testing::AssertionFailure()
                   << "AST nodes have different types: " << ast1.toString(false) << " vs " << ast2.toString(false);
        }
        ...
        else if (typeid(ast1) == typeid(const BinaryExpression &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "BinaryExpressions nodes have different operators: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        ...
        else if (typeid(ast1) == typeid(const Function &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "Function nodes have different identifiers: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
            if (f1.getReturnType() != f2.getReturnType())
            {
                return ::testing::AssertionFailure()
                       << "Function nodes have different return type: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        else if (typeid(ast1) == typeid(const FunctionParameter &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "FunctionParameter nodes have different identifiers: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
            if (f1.getParameterType() != f2.getParameterType())
            {
                return ::testing::AssertionFailure()
                       << "FunctionParameter nodes have different parameter type: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        ...
        else if (typeid(ast1) == typeid(const OperatorExpression &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "OperatorExpression nodes have different operators: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        ...
        else if (typeid(ast1) == typeid(const UnaryExpression &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "UnaryExpression nodes have different operators: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        else if (typeid(ast1) == typeid(const Variable &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "Variable nodes have different identifiers: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        else if (typeid(ast1) == typeid(const VariableDeclaration &))
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << "VariableDeclaration nodes have different datatypes: " << ast1.toString(false) << " vs "
                       << ast2.toString(false);
            }
        }
        else if (typeid(ast1) == typeid(const LiteralBool &))
        {
            ...
            {
                return ::testing::AssertionFailure() << "Literal nodes have different values: " << ast1.toString(false)
        ...
        else if (typeid(ast1) == typeid(const Call &))
        {
            ...
            bool equalArgs = std::equal(
                l1Args.begin(), l1Args.end(), l2.getArguments().begin(),
            ...
            {
                return ::testing::AssertionFailure() << "Call nodes have different values: " << ast1.toString(false)
                                                     << " vs " << ast2.toString(false);
            }
        }
        else
        ...
        if (ast1.countChildren() != ast2.countChildren())
            return ::testing::AssertionFailure()
                   << "Nodes do not have equal number of children!" << ast1.toString(false) << " has "
                   << ast1.countChildren() << " while " << ast2.toString(false) << " has " << ast2.countChildren();
        auto it1 = ast1.begin();
        auto it2 = ast2.begin();
        for (; it1 != ast1.end() && it2 != ast2.end(); ++it1, ++it2)
        {
            ...
            if (!r)
            {
                return ::testing::AssertionFailure() << ast1.toString(true) << " and " << ast2.toString(true)
                                                     ...
                                                     << it2->toString(false) << "Original issue:" << r.message();
            }
        }
