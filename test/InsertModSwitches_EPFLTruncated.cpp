...
class InsertModSwitches_EPFLTruncated : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<SealCiphertextFactory> scf_seal;
  std::unique_ptr<TypeCheckingVisitor> tcv;

  void SetUp() override {
    scf_seal = std::make_unique<SealCiphertextFactory>(16384);
...
// program's output
  ...
  if (!modSwitchVis.getModSwitchNodes().empty()) {
    ...
    rewritten_ast = modSwitchVis.insertModSwitchInAst(&rewritten_ast, binExprIns[4], coeffmodulusmap);
...
//    for (int i = 2; i < binExprIns.size(); i++) {
...
TEST_F(InsertModSwitches_EPFLTruncated, AdderInsertOneModSwitchNONTrunc) {
...
//    rewritten_ast = modSwitchVis.insertModSwitchInAst(&rewritten_ast, binExprIns[10], coeffmodulusmap);
//    rewritten_ast = modSwitchVis.insertModSwitchInAst(&rewritten_ast, binExprIns[363], coeffmodulusmap);
    ...
    // run the program and get its output
    auto map = tcv->getSecretTaintedNodes();
    RuntimeVisitor srv(*scf_seal, *astInput, map);
