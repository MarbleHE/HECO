...
TEST(ArithmeticExprTest, aexp3Add11) { /* NOLINT */
  ...
  auto *b = new ArithmeticExpr(valLhs, OpSymb::ArithmeticOp::addition, valRhs);
...
TEST(ArithmeticExprTest, aexp2Mult9Add3) { /* NOLINT */
  ...
  auto *b = new ArithmeticExpr(valLeft, OpSymb::ArithmeticOp::multiplication,
                               new ArithmeticExpr(valMid, OpSymb::ArithmeticOp::addition, valRight));
  // lhs: outer Aexp
  EXPECT_EQ(getLiteralIntValue(b->getLeft()), valLeft);
  EXPECT_EQ(b->getOp()->getOperatorString(), OpSymb::getTextRepr(OpSymb::ArithmeticOp::multiplication));
  ...
  EXPECT_EQ(r->getOp()->getOperatorString(), OpSymb::getTextRepr(OpSymb::ArithmeticOp::addition));
}
