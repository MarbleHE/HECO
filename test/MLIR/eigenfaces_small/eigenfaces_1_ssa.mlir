...
module  {
  func.func private @encryptedEigenfaces(%x: tensor<4x!fhe.secret<f64>>, %d: tensor<4x4xf64>) -> tensor<4x!fhe.secret<f64>> {
    ...
    %1 = affine.for %i = 0 to 4 iter_args(%cur0 = %0) -> (tensor<4x!fhe.secret<f64>>) {
      ...
      %2 = affine.for %j = 0 to 4 iter_args(%cur1 = %c0_sf64) -> !fhe.secret<f64>{
        ...
        %1 = tensor.extract %d[%i,%j] : tensor<4x4xf64>
        ...
        %5 = fhe.sub(%2, %4) : (f64, !fhe.secret<f64>) -> !fhe.secret<f64>         
