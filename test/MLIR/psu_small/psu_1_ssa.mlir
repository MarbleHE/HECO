...
module  {
  func.func private @encryptedPSU(%a_id: tensor<128x8x!fhe.secret<f64>>, %a_data: tensor<128x!fhe.secret<f64>>,
                                  %b_id: tensor<128x8x!fhe.secret<f64>>, %b_data: tensor<128x!fhe.secret<f64>>) -> !fhe.secret<f64> {
    %c0_sf64 = fhe.constant 0.000000e+00 : f64
    %c1_sf64 = fhe.constant 1.0 : f64

    // First, sum all of A
    %1 = affine.for %i = 0 to 128 iter_args(%cur0 = %c0_sf64) -> (!fhe.secret<f64>) {
        %1 = tensor.extract %a_data[%i] : tensor<128x!fhe.secret<f64>>
    ...
    %3 = affine.for %i = 0 to 128 iter_args(%cur1 = %1) -> (!fhe.secret<f64>) {
        // check if a[i] is a dupe of b[j]
        %4 = affine.for %j = 0 to 128 iter_args(%cur2 = %c1_sf64) -> !fhe.secret<f64>{
            // compute a_id[i] == b_id[j]
            %5 =  affine.for %k = 0 to 8 iter_args(%cur3 = %c1_sf64) -> !fhe.secret<f64>{
                %6 = tensor.extract %a_id[%i,%k] : tensor<128x8x!fhe.secret<f64>>
                %7 = tensor.extract %a_id[%j,%k] : tensor<128x8x!fhe.secret<f64>>
                %8 = fhe.sub(%6,%7) : (!fhe.secret<f64>, !fhe.secret<f64>) -> !fhe.secret<f64>
    ...
    %14 = tensor.extract %a_data[%i] : tensor<128x!fhe.secret<f64>>
