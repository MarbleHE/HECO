module  {
  func.func private @encryptedRobertsCross_64x64(%img: tensor<4096x!fhe.secret<i16>>) -> tensor<4096x!fhe.secret<i16>> {
    ...
    %r = affine.for %x = 0 to 64 iter_args(%imgx = %img) -> tensor<4096x!fhe.secret<i16>> {
      %1 = affine.for %y = 0 to 64 iter_args(%imgy = %imgx) -> tensor<4096x!fhe.secret<i16>> {
        ...
        // fetch img[x-1][y-1]
        %4 = arith.addi %x, %cm1 : index
        %5 = arith.muli %4, %c64 : index
        %6 = arith.addi %y, %cm1 : index
        ...
        // fetch img[x-1][y]
        %15 = arith.addi %x, %cm1 : index
        %16 = arith.muli %15, %c64 : index
        %17 = arith.addi %y, %cm1 : index
