...
from plot_utils import (
    get_x_ticks_positions,
    get_x_position,
    get_x_ticks_positions2,
    get_x_position2,
)
...
def extract_infos(folder, df_names, param_sizes=None):
    ...
    groups = sorted(list(groups), key=int)
    inner_groups = sorted(list(inner_groups))

    positions = {}

    for label in labels:
        g_idx = groups.index(label.split("_")[2])
...
def plot(
    labels: List[str],
    pandas_dataframes: List[pd.DataFrame],
    ...
    x_center, x_start = get_x_ticks_positions2(
        positions, bar_width, inner_spacer, spacer
    )
    ...
    fig.axes[0].get_yaxis().set_major_formatter(
        FuncFormatter(lambda x, p: human_format(x))
    )
    # add a grid
    ax = plt.gca()
    ax.grid(which="major", axis="y", linestyle=":")

    # MICROseconds to sec, name is misleading
    def ms_to_sec(num):
        return num / 1000000
    ...
    for i in range(len(labels)):
        if not labels[i] in positions:
            ...
            x_pos = get_x_position2(
                positions[labels[i]], x_start, bar_width, inner_spacer
            )
        ...
        p1 = plt.bar(x_pos, d1, bar_width, color=colors[0])
        d2 = ms_to_sec(df["t_input_encryption"].mean())
        d2_err = (
            0
            if math.isnan(df["t_input_encryption"].std())
            else df["t_input_encryption"].std()
        )
        p2 = plt.bar(x_pos, d2, bar_width, bottom=d1, color=colors[1])
        d3 = ms_to_sec(df["t_computation"].mean())
        d3_err = (
            0 if math.isnan(df["t_computation"].std()) else df["t_computation"].std()
        )
        p3 = plt.bar(x_pos, d3, bar_width, bottom=d1 + d2, color=colors[2])
        ...
        max_y_value = (
            d1 + d2 + d3 + d4 if (d1 + d2 + d3 + d4) > max_y_value else max_y_value
        )

        # Re-enable for error bars
        p4 = plt.bar(
            x_pos,
            d4,
            bar_width,
    ...
    # Add Legend (Re-enable for stacked bars)
    # plt.legend(
    #     (p4[0], p3[0], p2[0], p1[0]),
    ...
    # plt.ylim(0.001, max_y_rounded * 10)  # TODO [nku] need to scale this a bit
